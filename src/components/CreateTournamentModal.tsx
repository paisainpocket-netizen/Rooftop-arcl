...
export const CreateTournamentModal: React.FC<CreateTournamentModalProps> = ({
  ...
  // Resizes/compresses the uploaded banner before it is stored, so it
  // takes up only a few KB in Firebase instead of up to 2.5MB.
  const compressBannerImage = (file: File, maxDim = 500, quality = 0.75): Promise<string> => {
  ...
  const handleBannerUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    compressBannerImage(file, 500, 0.75)
      .then((result) => {
        setBannerImage(result);
        cricketAudio.playClick('Photo loaded');
      })
      .catch(() => {
        // Fallback to the original behaviour if compression fails.
        const reader = new FileReader();
        reader.onload = (event) => {
